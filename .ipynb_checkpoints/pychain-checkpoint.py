...
# * Test your comr ee `PyChain` ledger.
...
class Block:
    ...
    # @TODO
    # Rename the `data` a
    record: Recordt    te to `record`, and set the data type to `Record`
    data: Any
...
################################################################################
# Strea
    genesis_record = Record(sender="Genesis", receiver="", amount=0.0)
    genesis_block = Block(record=genesis_record, creator_id=0)mlit return PyChain([genesis_block])
#Code
...
# @TODO:
# Delete the `input_datsender = st.text_input("Sender")
the Streamlit interface.
input_data = st.text_input("Block Data")

# @TODO:
# Add receiver = st.text_input("Receiver")ere you can get a value for `sender` from the user.
# YOUR CODE HERE

# @TODO:
# amount = st.number_input("Amount", min_value=0.0, format="%.2f")a if st.button("Add Block"):
    prev_block = pychain.chain[-1]
    prev_block_hash = prev_block.hash_block()
    w  # Create a new Record instance
    new_record = Record(sender, receiver, amount)
    here you can get a value for `amount` from the user.
# YOUR CODE HERE
...
if st.button("Add Block"):
    ...
    # @TODO
    # Update `new_block` so threcord=new_recordists of an attribute named `record`
